...
void BSTY::adjustHeights(NodeT *n) {
	if (n != NULL) {
		...
		if ((left > right) && (n->height != left + 1))
			n->height = left + 1;
		else if ((right > left) && (n->height != right + 1))
			n->height = right + 1;
		else if ((n->height != right + 1) && (right == left))
			n->height = right + 1;

		// Do rotations before recursive call
		if (findBalance(n) == 2) {
			// do shit (left rotations)
		} else if (findBalance(n) == -2) {
			// do more shit (right rotations)
		}
...
int BSTY::findBalance(NodeT* n) {
	...
	// Check to make sure n isn't a leaf node
	if (n->height > 1) {
		if ((n->left == NULL) && (n->right != NULL))
